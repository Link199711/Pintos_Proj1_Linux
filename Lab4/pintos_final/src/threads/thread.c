...
bool
cmp_thread(const struct list_elem *thread1, const struct list_elem *thread2, void *aux UNUSED)
{
...
bool
cmp_lock(const struct list_elem *lock1, const struct list_elem *lock2, void *aux UNUSED)
{
...
update_thread_priority (struct thread *t)
{
  ...
  if (!list_empty (&t->locks))
  {
    ...
    if (list_entry (list_front (&t->locks), struct lock, elem)->priority > max_priority)
      max_priority = list_entry (list_front (&t->locks), struct lock, elem)->priority;
  }
...
void renew_priority (struct thread *t)
{
  ...
  temp3 = fp_to_int_round_nearest (fp_sub (temp3, temp2));
